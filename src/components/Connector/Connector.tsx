...
export default function Connector({ row, network, connector, setIsEditModalOpen, setValues }: { row: number, network: NetworkType, connector: ConnectorType[], setIsEditModalOpen: (value: boolean) => void, setValues: (value: ConnectorType) => void }) {
  ...
  return (
    <div className="w-full p-2 mb-2  border-b-1 border-2 border-gray-400 rounded hover:border-2 hover:border-r-2 hover:border-black hover:shadow-lg hover:shadow-lg" style={{
      ...
      <div className="flex flex-wrap mt-3 justify-center">
        {/* Map over the connectors array */}
        {connector.map((singleConnector, conIndex) => (
          <div
            key={conIndex}
          ...
          >
            <label className="inline-flex items-center">
              <div className='rounded-full h-10 w-10 flex items-center justify-center text-white text-bold text-lg bg-gray-900'>
                {singleConnector.name[0]}
              </div>
            </label>
            <div className="text-left ml-2 flex-grow">
              <p className="text-xs font-bold text-blue-500 truncate w-32">{singleConnector.name}</p>
              <p className="text-xs text-gray-500">
                <span className="font-bold">IP: {singleConnector.ip}</span>
              </p>
            </div>
